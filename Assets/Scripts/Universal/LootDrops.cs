...
public class LootDrops : MonoBehaviour
{
    ...
    [SerializeField] List<ItemSO> commonDrops, rareDrops, singleDrops;

    public List<ItemSO> CommonDrops { get => commonDrops; set => commonDrops = value; }
    public List<ItemSO> RareDrops { get => rareDrops; set => rareDrops = value; }
    public List<ItemSO> SingleDrops { get => singleDrops; set => singleDrops = value; }
    ...
    void CreateItemDrop(ItemSO drop)
    {
    ...
    public void SpawnCommonDrops()
    {
        ...
        {
            ItemSO drop = commonDrops[Random.Range(0, commonDrops.Count)];

            for (int j = 0; j < (drop as EquipmentSO ? 1 : Random.Range(1, 3)); j++)
            {
                CreateItemDrop(drop);
            }
        }
